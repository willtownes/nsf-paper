...
```{r}
ggplot(d,aes(x=model,y=dev_val_mean,fill=simulation,color=simulation))+geom_boxplot()+scale_y_log10()+ylab("validation deviance (mean)")
...
ggplot(d,aes(x=model,y=spatial_importance_dist,fill=simulation,color=simulation))+geom_boxplot()+ylab("spatial importance distance from truth")
