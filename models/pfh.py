...
# -*- coding: utf-8 -*-
"""
Process Factorization- Hybrid version with both spatial and nonspatial factors
...
class ProcessFactorizationHybrid(tf.Module):
  def __init__(self, N, J, L, Z, T=None, lik="poi", nonneg=True,
    ...
    self.nonneg = nonneg
    self._disp0 = disp
    self.spat = pf.ProcessFactorization(J, T, Z, nonneg=nonneg, lik=lik,
                                        isotropic=isotropic,
  ...
  def init_loadings(self,Y,X=None,sz=1,**kwargs):
    ...
    L,T = self.get_dims()
    if self.nonneg:
      init_npfh_with_nmf(self,Y,X=X,sz=sz,**kwargs)
  ...
  def generate_pickle_path(self,sz,base=None):
    ...
    pars = {"L":L, "T":T, "lik":self.lik, "sz":sz,
            "model":"NPFH" if self.nonneg else "RPFH",
  ...
  def sample_predictive_mean(self, X, idx=None, sz=1, S=1, kernel=None,
    ...
    """
    s_cached_vars: spatial cached vars, see method of NPF class
...
def init_npfh_with_nmf(fit, Y, X=None, sz=1, pseudocount=1e-2, factors=None,
  ...
  U,beta0,beta = pf.smooth_spatial_factors(FH[:,:T], fit.spat.Z.numpy(), X=X)
