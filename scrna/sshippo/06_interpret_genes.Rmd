...
loadings from NSFH
```{r}
W<-read.csv(fp(pth,"results/sshippo_nsfh20_spde_loadings.csv"),header=TRUE,row.names=1)
...
res$dim<-rep(1:10,2)
write.csv(res[,c(1,4,2,3)],fp(pth,"results/sshippo_nsfh20_spde_goterms.csv"),row.names=FALSE)
```
...
```{r}
res<-read.csv(fp(pth,"results/sshippo_nsfh20_spde_goterms.csv"),header=TRUE)
...
write.csv(res,fp(pth,"results/sshippo_nsfh20_spde_goterms.csv"),row.names=FALSE)
```
Shorter GO table
```{r}
res<-read.csv(fp(pth,"results/sshippo_nsfh20_spde_goterms.csv"),header=TRUE)
...
write.csv(res2,fp(pth,"results/sshippo_nsfh20_spde_goterms_short.csv"),row.names=FALSE)
```
...
write.csv(hres,fp(pth,"results/sshippo_hotspot_goterms.csv"),row.names=FALSE)
hres2<-hres
...
write.csv(hres2,fp(pth,"results/sshippo_hotspot_goterms_short.csv"),row.names=FALSE)
```
