...
loadings from NSFH
```{r}
W<-read.csv(fp(pth,"results/xyz_liv_nsfh6_spde_loadings.csv"),header=TRUE,row.names=1)
...
res$type<-rep(c("spat","nsp"),each=3)
res$dim<-rep(1:3,2)
write.csv(res[,c(1,4,2,3)],fp(pth,"results/xyz_liv_nsfh6_spde_goterms.csv"),row.names=FALSE)
```
...
```{r}
res<-read.csv(fp(pth,"results/xyz_liv_nsfh6_spde_goterms.csv"),header=TRUE)
...
mk<-subset(mk,organ %in% c("Liver","Immune system","Vasculature","Blood"))
pg<-tapply(mk$official.gene.symbol,mk$cell.type,function(x){x},simplify=FALSE)
ct<-lapply(res$genes,genes2celltype,pg,ss=1)
ct[is.na(ct)]<-""
#the automated search did not work so we tried a manual search on the panglao website
ct[[1]]<-"Hepatocytes"
ct[[3]]<-"Macrophages"
ct[[4]]<-"Fibroblasts"
ct[[6]]<-"Macrophages"
res$celltypes<-paste(ct,sep=", ")
write.csv(res,fp(pth,"results/xyz_liv_nsfh6_spde_goterms.csv"),row.names=FALSE)
```
Shorter GO table
```{r}
res<-read.csv(fp(pth,"results/xyz_liv_nsfh6_spde_goterms.csv"),header=TRUE)
...
write.csv(res2,fp(pth,"results/xyz_liv_nsfh6_spde_goterms_short.csv"),row.names=FALSE)
```
...
write.csv(hres,fp(pth,"results/xyz_liv_hotspot_goterms.csv"),row.names=FALSE)
hres2<-hres
...
write.csv(hres2,fp(pth,"results/xyz_liv_hotspot_goterms_short.csv"),row.names=FALSE)
```
